...
- **당일 반대매매 제한**:
  - 한 종목을 매수한 날에는 그 종목에 대한 매도 불가
  - 반대로 매도한 날에는 그 종목에 대한 매수 불가
  - (주식 시장 규칙, 코인은 제한 없음)
...
- [x] 모멘텀 스코어 계산:
  - [x] 모멘텀 스코어1 = 100일 평균 모멘텀 (10일마다) - 장기추세
  - [x] 모멘텀 스코어2 = 20일간 등락률의 이동평균선 - 단기추세
- [x] 비중 조절:
  - [x] IF 레버리지의 스코어1, 스코어2 모두 > 인버스: 레버리지 × 1.3, 인버스 × 0.7
  - [x] ELSE: 레버리지 × 0.7, 인버스 × 1.3
...
**시간 기준 정의**
- **하루 기준**: 한국시간 09:00 (UTC 00:00)
  - 전일 = 어제 09:00 ~ 오늘 08:59
  - 금일 시가 = 오늘 09:00 첫 1분봉 시가
  - 전일 고가/저가 = 어제 09:00 ~ 오늘 08:59의 고가/저가

**코인 특화 설정**
- 당일 반대매매 제한: 없음 (24시간 자유 거래)
- 슬리피지: 0.01~0.05% (거래소 수수료)
- 펀딩비: 롱/숏 포지션 보유 시 8시간마다 정산 고려
- 레버리지: 3~5배 권장 (리스크 관리)
- 청산가 관리: 필수 (급락 시 강제청산 방지)

**실시간 거래 (1분봉 + WebSocket)**
- 실시간으로 가격 수신 시 목표가 돌파 즉시 주문
- 일봉 기준 지표(EMA 60일, RSI 등)는 별도 계산
- 1분봉 기준 지표는 기간 재조정 (예: 1440분 = 1일)
