...
- 당일 거래량이 전일 거래량 대비 500% 이상 증가 시 매수
- 주가 1,000원 이상만 거래
- 양봉이어야 함 (종가 > 시가)
...
class BalloonTheoryStrategy(Strategy):
    ...
    volume_multiplier = 5.0   # 거래량 증가율 (500%)
    min_volume = 500000        # 최소 거래량 (50만개)
    ...
    def init(self):
        ...
        # 진입 시 정보 저장 (손절/매도 기준용)
        self.entry_prev_open = None  # 매수일 전일 시가 (손절 기준)
    ...
    def _check_buy_signal(self, close, open_price, prev_open, volume, prev_volume):
        ...
        # 모든 조건 만족 시 매수
        self.buy()
        self.entry_prev_open = prev_open  # 손절 기준 저장 (전일 시가)
